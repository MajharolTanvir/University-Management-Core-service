...
const updateCourse = async (id: string, payload: ICourseCreatedData) => {
  const { preRequisiteCourses, ...courseData } = payload;

  await prisma.$transaction(async courseTransaction => {
    const result = await courseTransaction.course.update({
      where: {
        id,
      },
      data: courseData,
    ...
    if (preRequisiteCourses && preRequisiteCourses.length > 0) {
      ...
      await asyncForEach(
        deletePrerequisite,
        async (deletePreCourses: IPrerequisiteCourseRequest) => {
          await courseTransaction.courseToPrerequisite.deleteMany({
            where: {
              AND: [
                {
                  courseId: id,
                ...
                {
                  preRequisiteId: deletePreCourses.courseId,
                },
              ],
            },
          });
